...
const MobileTabs = () => {
  ...
  return (
    <>
      <div className="lg:hidden flex bg-white shadow-lg fixed bottom-0 z-10 w-full items-center justify-between md:justify-evenly text-gray-500">
        {allTypes.map((item) => {
          return (
            <Link key={item.id} to={`/${item.id}`} className="w-1/4">
              <div
                className={
                  type === item.id
                    ? "flex items-center text-center flex-col relative text-zomato-400"
            ...
            <Link key={item.id} to={`/${item.id}`} className="w-36">
              <div
                className={classnames(
                  "flex gap-3 pb-2 items-center transition duration-700 ease-in-out",
                  {
              ...
              >
                <div
                  className={classnames(
                    "w-16 h-16 bg-gray-100 rounded-full p-4",
                    {
